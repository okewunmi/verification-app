...
//         throw new Error('Scanner not available. Please ensure Windows Hello is enabled.');
//       }

//       // Capture fingerprint using WebAuthn
//       const captureResult = await fingerprintScanner.capture(
//         selectedStudent.$id,
//         `${selectedStudent.matricNumber}_${currentFinger.id}`
//       );
...
//             quality: 95, // WebAuthn doesn't provide quality score
//             capturedAt: new Date().toISOString()
//           }
//         }));
...
//         setCaptureStatus({ 
//           type: 'success', 
//           message: `${currentFinger.label} captured successfully!` 
//         });
...
//         {/* Create/Edit Student Modal - CONTINUES IN NEXT PART */}
//               {/* Create/Edit Student Modal */}
//         {showModal && (
//           <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
//             <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
...
export default function StudentManagement() {
  ...
  // UPDATED: DigitalPersona capture with duplicate check
  const handleCaptureFinger = async () => {
    if (!scannerReady) {
    ...
    try {
      ...
      // CRITICAL: Check for duplicates before accepting
      setCheckingDuplicates(true);
      setCaptureStatus({ type: 'info', message: 'Checking for duplicates...' });
      
      // Get all existing fingerprint templates
      const existingTemplates = [];
      for (const student of students) {
        if (student.$id === selectedStudent.$id) continue; // Skip current student
      ...
      // Also check already captured fingers in this session
