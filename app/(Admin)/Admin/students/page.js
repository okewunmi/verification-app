...
//                   <div>
//                     <h2 className="text-2xl font-bold text-white">DigitalPersona Fingerprint Capture</h2>
...
//                         captureStatus.type === 'success' ? 'bg-green-50 border-2 border-green-200' : 
//                         captureStatus.type === 'error' ? 'bg-red-50 border-2 border-red-200' :
//                         'bg-blue-50 border-2 border-blue-200'
//                       }`}>
//                         <p className={`font-medium ${
//                           captureStatus.type === 'success' ? 'text-green-700' : 
//                           captureStatus.type === 'error' ? 'text-red-700' :
//                           'text-blue-700'
//                         }`}>
...
//                           <p className="font-medium text-yellow-700">Checking for duplicate fingerprints...</p>
//                         </div>
//                       </div>
//                     )}
...
//                         (isCapturing || checkingDuplicates) ? 'opacity-50 cursor-not-allowed' : ''
//                       }`}
//                     >
//                       {checkingDuplicates ? (
...
//                       <p className="font-semibold mb-1">Duplicate Detection Enabled</p>
//                       <p>Each fingerprint is checked against all registered students to prevent duplicates. If a duplicate is detected, you'll be prompted to use a different finger.</p>
//                     </div>
//                   </div>
//                 </div>
//               </div>
...
export default function StudentManagement() {
  ...
  // CRITICAL FIX: HMR-resistant fingerprint state using Refs
  const scannerInitializedRef = useRef(false);
  const capturedFingersRef = useRef({});
  const currentFingerIndexRef = useRef(0);
  const capturingRef = useRef(false);
  
  // UI state (can refresh without losing data)
  const [uiRefresh, setUiRefresh] = useState(0);
  const [captureStatus, setCaptureStatus] = useState(null);
  const [scannerStatus, setScannerStatus] = useState({ ready: false, message: null, type: null });
  const [checkingDuplicates, setCheckingDuplicates] = useState(false);

  // Force UI update helper
  const forceUIUpdate = () => setUiRefresh(prev => prev + 1);
  ...
  const openFingerprintModal = async (student) => {
    ...
    // Initialize scanner only once per session
    if (scannerInitializedRef.current) {
      console.log('✅ Scanner already initialized - reusing connection');
      setScannerStatus({ ready: true, message: 'Scanner ready! Click "Capture" to begin.', type: 'success' });
      return;
    }
  ...
  const closeFingerprintModal = async () => {
    ...
    // DON'T reset these - preserve for next capture
    // scannerInitializedRef.current = false;
    // capturedFingersRef.current = {};
    // currentFingerIndexRef.current = 0;
  };
  ...
  const handleCaptureFinger = async () => {
    ...
    if (capturingRef.current) {
      console.warn('⚠️ Already capturing - ignoring duplicate click');
      return;
    }
    ...
    try {
      const currentFinger = fingers[currentFingerIndexRef.current];
      
      console.log(`📸 Starting capture for: ${currentFinger.label}`);
      
      // Capture fingerprint
      const captureResult = await fingerprintScanner.capture(currentFinger.label);

      if (!captureResult.success) {
        throw new Error(captureResult.error);
      }

      console.log('✅ Capture successful, checking for duplicates...');
      ...
      // Add fingerprints from OTHER students
      for (const student of students) {
        if (student.$id === selectedStudent.$id) continue;
        
        const templates = [
          { template: student.thumbTemplate, name: 'Thumb' },
          { template: student.indexTemplate, name: 'Index' },
          { template: student.middleTemplate, name: 'Middle' },
          { template: student.ringTemplate, name: 'Ring' },
          { template: student.pinkyTemplate, name: 'Pinky' }
        ];
      ...
      // CRITICAL: Check against current session prints
      Object.entries(capturedFingersRef.current).forEach(([fingerId, data]) => {
        if (fingerId !== currentFinger.id) {
          existingTemplates.push({
            studentId: selectedStudent.$id,
            matricNumber: selectedStudent.matricNumber,
            template: data.template,
            fingerName: `${fingers.find(f => f.id === fingerId)?.label || fingerId} (Current Session)`,
            studentName: `${selectedStudent.firstName} ${selectedStudent.surname}`
          });
        }
      });

      console.log(`🔍 Checking against ${existingTemplates.length} fingerprints`);
      ...
      if (duplicateCheck.hasDuplicates) {
        ...
        setCaptureStatus({ 
          type: 'error', 
          message: `⚠️ DUPLICATE! This fingerprint matches: ${duplicate.studentName || duplicate.matricNumber} (${duplicate.fingerName}). Please use a different finger.` 
        });
      ...
      // No duplicate - save to ref
      console.log('✅ Fingerprint is unique - accepting');
      
      capturedFingersRef.current[currentFinger.id] = {
        template: captureResult.template,
        quality: captureResult.quality,
        capturedAt: captureResult.capturedAt
      };
      ...
      // Move to next finger
      setTimeout(() => {
        if (currentFingerIndexRef.current < fingers.length - 1) {
          currentFingerIndexRef.current++;
          setCaptureStatus({ type: 'info', message: 'Ready for next finger...' });
          capturingRef.current = false;
          forceUIUpdate();
        } else {
          // All 5 fingers captured
          saveFingerprintsToDatabase();
        }
      }, 1500);
  ...
  const saveFingerprintsToDatabase = async () => {
    ...
    try {
      const fingerprintData = {
        thumb: capturedFingersRef.current.thumb?.template || '',
        index: capturedFingersRef.current.index?.template || '',
        middle: capturedFingersRef.current.middle?.template || '',
        ring: capturedFingersRef.current.ring?.template || '',
        pinky: capturedFingersRef.current.pinky?.template || ''
      };
  ...
  const capturedCount = Object.keys(capturedFingersRef.current).length;
  const captureProgress = (capturedCount / 5) * 100;
  const isCapturing = capturingRef.current;
  const currentFingerIndex = currentFingerIndexRef.current;
              ...
              <div className="p-6">
                ...
                {scannerStatus.type === 'error' && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-red-600 font-medium">{scannerStatus.message}</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-5 gap-2 mt-6">
                  {fingers.map((finger, index) => {
                    const isCaptured = capturedFingersRef.current[finger.id];
                    const isCurrent = index === currentFingerIndex && !isCaptured;
                    ...
                    return (
                      ...
                      >
                        ...
                        {isCaptured && (
                          <div className="mt-1">
                            <span className="text-xs text-green-600 font-semibold">
                              ✓ {capturedFingersRef.current[finger.id].quality}%
                            </span>
                          </div>
                        )}
                      </div>
                ...
                <div className="mt-6 p-4 bg-yellow-50 border-2 border-yellow-200 rounded-lg">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-sm text-yellow-800">
                      <p className="font-semibold mb-1">Duplicate Detection Enabled</p>
                      <p>Each fingerprint is checked against all registered students AND previously captured fingers in this session to prevent duplicates.</p>
                    </div>
                  </div>
                </div>
              </div>
