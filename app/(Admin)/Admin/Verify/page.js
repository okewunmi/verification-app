...
//         video: {
//           facingMode: facingMode, // Use state instead of hardcoded 'user'
//           width: { ideal: 1280 },
//           height: { ideal: 720 }
//         }
//       });
...
//       setCameraActive(true);
//       setStatus({ 
//         message: `✅ Switched to ${newFacingMode === 'user' ? 'front' : 'back'} camera`, 
//         type: 'success' 
//       });
//       console.log(`✅ Camera switched to ${newFacingMode === 'user' ? 'front' : 'back'}`);
//     } catch (err) {
//       console.error('Camera toggle error:', err);
//       setErrorMessage('Unable to switch camera. Please try again.');
//       setStatus({ message: 'Camera toggle failed', type: 'error' });
//     }
//   };
...
export default function OptimizedExamVerification() {
  ...
  useEffect(() => {
    ...
    const detectFaceInRealTime = async () => {
      if (!cameraActive || !videoRef.current || !faceRecognition || !modelsLoaded) return;
  ...
  useEffect(() => {
    ...
    const loadFaceRecognition = async () => {
      try {
        ...
        console.error('Face recognition load error:', error);
        if (mounted) setStatus({ message: 'Face recognition unavailable. Please refresh.', type: 'error' });
      }
    };
  ...
  const handleFaceVerification = async () => {
    ...
    if (!faceDetected) {
      setStatus({ message: '⚠️ No face detected. Please ensure your face is visible.', type: 'warning' });
      return;
    }
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        if (!faceRecognition) throw new Error('Face recognition library not initialized');
        ...
        const capturedImageBase64 = await captureFaceImage();
        if (!capturedImageBase64) throw new Error('Failed to capture image from video');
        ...
        if (!studentsResult.success || studentsResult.data.length === 0) {
          setVerificationResult({ matched: false, message: '⚠️ No registered faces in database.' });
          setStatus({ message: 'No registered faces found', type: 'warning' });
          break;
        }
        ...
        if (verifyResult.success && verifyResult.matched) {
          ...
          setVerificationResult({ matched: false, message: verifyResult.message || 'No matching student found' });
          setStatus({ message: 'No match found', type: 'error' });
          new Audio('/sounds/error.mp3').play().catch(() => {});
        }
        break;
        ...
        console.error(`❌ Face verification attempt ${attempt}/${MAX_RETRIES} failed:`, err);
        if (attempt < MAX_RETRIES) {
          setStatus({ message: `Retrying... (${attempt}/${MAX_RETRIES})`, type: 'warning' });
  ...
  const handleFingerprintVerification = async () => {
    ...
    if (!nbisReady) {
      setStatus({ message: '🔥 Waking up fingerprint server... (first use takes ~30s)', type: 'info' });
    } else {
      setStatus({ message: 'Place your finger on the scanner...', type: 'info' });
    }
  ...
  const handleStartVerification = async () => {
    if (!verificationType) { alert('Please select a verification method'); return; }
    if (verificationType === 'Face') await handleFaceVerification();
    else if (verificationType === 'Fingerprint') await handleFingerprintVerification();
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Fast biometric authentication with real-time face detection</p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm border ${
              ...
              {nbisReady ? <CheckCircle className="w-4 h-4" /> : <Loader2 className="w-4 h-4 animate-spin" />}
              ...
              <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {!nbisReady && (
                  <div className="mt-3 flex items-center gap-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg text-xs text-yellow-700">
                    ...
                    <span>Fingerprint server is waking up — first verification may take up to 30s longer</span>
                  </div>
                )}
              </div>
            )}
            ...
            {verificationResult?.matched && verificationResult?.student && (
              <div className="text-center py-4">
                ...
                <div className="bg-green-50 rounded-2xl p-6 border-4 border-green-200">
                  <h3 className="text-3xl font-bold text-green-800 mb-4">✓ MATCH FOUND!</h3>
                  ...
                  <div className="space-y-3 text-left max-w-md mx-auto">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white p-4 rounded-lg shadow">
                        <p className="text-sm text-gray-600 mb-1">Department</p>
                        <p className="font-semibold text-gray-900">{verificationResult.student.department}</p>
                      </div>
                      <div className="bg-white p-4 rounded-lg shadow">
                        <p className="text-sm text-gray-600 mb-1">Level</p>
                        <p className="font-semibold text-gray-900">{verificationResult.student.level}</p>
                      </div>
                    </div>
                    ...
                    {verificationResult.verificationTime && (
                      <div className="bg-white p-3 rounded-lg shadow text-center">
                        <span className="text-sm text-gray-500">⚡ Verified in </span>
                        <span className="font-bold text-indigo-600">{verificationResult.verificationTime}s</span>
                        {verificationResult.fingerName && (
                          <span className="text-sm text-gray-500"> · {verificationResult.fingerName} finger</span>
                        )}
                      </div>
                    )}
                  </div>
