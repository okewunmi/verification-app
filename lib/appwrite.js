...
const EMAILJS_CONFIG = {
  serviceId: 'service_b84su78', // Replace with your EmailJS service ID
  templateId: 'template_t68c8no', // Replace with your EmailJS template ID
  publicKey: 'xFhVsuxvqbl4YC_1X' // Replace with your EmailJS public key
};
...
export const sendWelcomeEmail = async (studentData, matricNumber, defaultPassword) => {
  try {
    const templateParams = {
      ...
      to_name: `${studentData.firstName} ${studentData.surname}`,
      student_name: `${studentData.firstName} ${studentData.surname}`,
...
export const createStudent = async (studentData, profilePictureFile = null) => {
  try {
    ...
    // 👇 ADD THIS SECTION - Send welcome email with credentials
    ...
    return {
      ...
      data: student,
      matricNumber,
      emailSent: emailResult.success, // 👈 ADD THIS
      credentials: {
        username: matricNumber,
        defaultPassword: studentData.surname,
        message: emailResult.success  // 👈 ADD THIS
          ? 'Login credentials sent to email' 
          : 'Login credentials created (email failed to send)'
      }
    };
  } catch (error) {
    console.error('Error creating student:', error);
