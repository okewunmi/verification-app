...
export const saveFingerprints = async (studentId, fingerprintData) => {
  try {
    ...
    // DEBUG: Log the entire fingerprintData object
    console.log('📦 Received fingerprintData:', fingerprintData);
    console.log('📦 fingerprintData type:', typeof fingerprintData);
    console.log('📦 fingerprintData keys:', Object.keys(fingerprintData || {}));
    
    // Log each finger's data
    if (fingerprintData) {
      Object.entries(fingerprintData).forEach(([key, value]) => {
        console.log(`  - ${key}:`, typeof value, value ? `(${value.length} chars)` : 'empty');
      });
    }
    ...
    // FIX: Check if template is a string before using .trim()
    const capturedFingers = Object.entries(fingerprintData).filter(
      ([key, template]) => {
        // Ensure template is a string and not empty
        if (typeof template === 'string') {
          return template.trim() !== '';
        }
        // If it's not a string but exists, log warning and skip
        if (template) {
    ...
    for (const finger of fingers) {
      // FIX: Verify template is a string before processing
      if (finger.template && typeof finger.template === 'string' && finger.template.trim() !== '') {
        try {
          ...
          const uploadResult = await storage.createFile(
            config.bucketId, // Same bucket ID
            ID.unique(),
            file
          );
        ...
        // Log if template exists but is not a string
