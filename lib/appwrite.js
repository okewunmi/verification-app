...
//     // Calculate upcoming exams (you'll need to create exam sessions collection)
//     // For now, returning 0 - implement when exam sessions are ready
 ...
 */
/**
 * Get recent activity from the database (UPDATED)
 */
...
/**
 * Create course registration collection document
 */
export const registerStudentCourses = async (studentId, matricNumber, courses, semester) => {
  try {
    const registrations = [];
    const errors = [];
    ...
    return {
      ...
      data: registrations,
      errors: errors,
      message: `${registrations.length} course(s) registered successfully${errors.length > 0 ? `, ${errors.length} failed` : ''}`
    };
  } catch (error) {
