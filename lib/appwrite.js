...
export const registerStudentCourses = async (studentId, matricNumber, courses, academicYear, semester) => {
  try {
    ...
    console.log(`Starting registration for ${courses.length} courses for ${matricNumber}`);
    
    // First, get all existing registrations for this student to check duplicates
    ...
    // Create a Map of already registered course codes with their full data for fast lookup
    const registeredCourses = new Map();
    existingRegs.documents.forEach(reg => {
      registeredCourses.set(reg.courseCode, reg);
    });

    console.log('Already registered courses:', Array.from(registeredCourses.keys()));
    console.log('Courses to register:', courses.map(c => c.courseCode));

    // Process each course sequentially to avoid race conditions
    for (let i = 0; i < courses.length; i++) {
      const course = courses[i];
      
      try {
        // Check if course is already registered
        if (registeredCourses.has(course.courseCode)) {
          console.log(`Skipping ${course.courseCode} - already registered`);
          skipped.push({
        ...
        console.log(`[${i + 1}/${courses.length}] Registering ${course.courseCode}...`);
        ...
        const registration = await databases.createDocument(
          config.databaseId,
          config.courseRegistrationCollectionId,
          uniqueId, // Explicitly use unique ID
          {
            studentId: studentId,
        ...
        // Add to the map immediately so we don't try to register it again in this batch
        registeredCourses.set(course.courseCode, registration);
        console.log(`✓ Successfully registered: ${course.courseCode}`);
        ...
        if (error.code === 409 || 
          ...
          console.log(`→ Duplicate detected for ${course.courseCode}`);
          skipped.push({
            course: course.courseCode,
            reason: 'Already registered (duplicate detected)'
          });
        } else {
          console.log(`→ Failed to register ${course.courseCode}`);
          errors.push({
            course: course.courseCode,
    ...
    if (registrations.length > 0) {
      parts.push(`${registrations.length} course(s) registered successfully`);
    }
    if (skipped.length > 0) {
    ...
    message = parts.length > 0 ? parts.join(', ') : 'No courses registered';
    ...
    console.error('Fatal error in registerStudentCourses:', error);
    return { 
      success: false, 
