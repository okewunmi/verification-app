...
export const registerStudentCourses = async (studentId, matricNumber, courses, academicYear, semester) => {
  try {
    ...
    console.log(`Starting registration for ${courses.length} courses`);
    ...
    // Create a Set of already registered course codes for fast lookup
    const registeredCourseCodes = new Set(
      existingRegs.documents.map(reg => reg.courseCode)
    );

    console.log('Already registered courses:', Array.from(registeredCourseCodes));
    ...
    for (const course of courses) {
      try {
        ...
        console.log(`Registering ${course.courseCode}...`);
        ...
        // Add to the set immediately so we don't try to register it again in this batch
        registeredCourseCodes.add(course.courseCode);
        ...
        if (error.code === 409 || 
          ...
          skipped.push({
            course: course.courseCode,
            reason: 'Already registered (duplicate)'
          });
        } else {
          errors.push({
            course: course.courseCode,
