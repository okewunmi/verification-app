...
// ========================================
// FINGERPRINT FUNCTIONS - DIGITALPERSONA 4500
// ========================================

/**
 * Save fingerprint templates to student record
 * Called after capturing fingerprints with DigitalPersona scanner
 */
export const saveFingerprints = async (studentId, fingerprintData) => {
  try {
    ...
    // Validate that at least one fingerprint was captured
    const hasFinger = Object.values(fingerprintData).some(template => template && template.trim() !== '');
    ...
    const student = await databases.updateDocument(
      ...
      studentId,
      {
        // Store Base64-encoded FMD (Fingerprint Minutiae Data) templates
...
/**
 * Get all students with fingerprints (for verification)
 * This runs SERVER-SIDE to fetch data from database
 */
export const getStudentsWithFingerprints = async () => {
  try {
    ...
    console.log(`📝 Prepared ${templates.length} fingerprint templates`);
...
export const verifyStudentFingerprint = async (fingerprintScannerInstance) => {
  try {
    ...
    // Step 1: Get all stored fingerprint templates from database
    console.log('📥 Fetching stored templates...');
    ...
    if (!templatesResult.success) {
      return {
        success: false,
        matched: false,
        error: 'Failed to fetch stored fingerprints: ' + templatesResult.error
      };
    }
    ...
    console.log(`📋 Found ${templatesResult.data.length} stored fingerprint templates`);

    // Step 2: Capture fingerprint from scanner (CLIENT-SIDE)
    ...
    if (!captureResult.success) {
      return {
        success: false,
        matched: false,
        error: 'Failed to capture fingerprint: ' + captureResult.error
      };
    }
    ...
    // Step 3: Compare captured fingerprint with all stored templates
    ...
    for (let i = 0; i < templatesResult.data.length; i++) {
      ...
      try {
        console.log(`  [${i + 1}/${templatesResult.data.length}] Comparing with ${stored.matricNumber} (${stored.fingerName})...`);
        
        // Compare templates using DigitalPersona SDK
        ...
        if (compareResult.success && compareResult.matched) {
          console.log(`    ✅ MATCH! Score: ${compareResult.score}, Confidence: ${compareResult.confidence.toFixed(1)}%`);
          
          // Keep track of best match (lowest score = best match in DigitalPersona)
          ...
          console.log(`    ❌ No match (score: ${compareResult.score})`);
        }
    ...
    if (bestMatch) {
      ...
      console.log(`Finger Used: ${bestMatch.fingerName}`);
      console.log(`Confidence: ${highestConfidence.toFixed(1)}%`);
      console.log(`Match Score: ${lowestScore}`);
      console.log('===================================\n');
...
export const checkFingerprintDuplicate = async (fingerprintScannerInstance, newTemplate) => {
  try {
    ...
    const DUPLICATE_THRESHOLD = 20000; // DigitalPersona threshold
    ...
    for (const stored of templatesResult.data) {
      ...
      if (compareResult.matched && compareResult.score <= DUPLICATE_THRESHOLD) {
        ...
        return {
          ...
          message: `This fingerprint is already registered to ${stored.student.firstName} ${stored.student.surname} (${stored.matricNumber})`
        };
      }
    }
...
/**
 * Search for matching student using face verification API
 * This function calls the Next.js API route
 */
// export const searchStudentByFace = async (capturedImageBase64) => {
//   try {
...
//         'Content-Type': 'application/json',
//       },
//       body: JSON.stringify({ capturedImageBase64 })
//     });
