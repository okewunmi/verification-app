...
export const registerStudentCourses = async (studentId, matricNumber, courses, academicYear, semester) => {
  try {
    ...
    for (const course of courses) {
      try {
        // Get the full course semester from the course object or use the passed semester
        ...
        const registration = await databases.createDocument(
          ...
          ID.unique(),
          {
            ...
            courseUnit: parseInt(course.courseUnit), // Ensure it's an integer
            semester: courseSemester, // Use course semester or fallback
            status: 'Pending',
    ...
    return {
      ...
      message: errors.length === 0 
        ? `${registrations.length} course(s) registered successfully` 
        : `${registrations.length} registered, ${errors.length} failed: ${errors.map(e => e.course).join(', ')}`
    };
  } catch (error) {
