...
// ========================================
// PRODUCTION-READY FINGERPRINT FUNCTIONS
// For DigitalPersona 4500 Scanner
// ========================================

// /**
//  * Save fingerprint templates to student record
//  * @param {string} studentId - Student document ID
//  * @param {Object} fingerprintData - Object with thumb, index, middle, ring, pinky templates
...
//       return { success: false, error: 'Invalid fingerprint data' };
//     }

//     // Validate at least one fingerprint exists
//     const templates = [
//       fingerprintData.thumb,
//       fingerprintData.index,
//       fingerprintData.middle,
//       fingerprintData.ring,
//       fingerprintData.pinky
//     ].filter(t => t && t.trim() !== '');

//     if (templates.length === 0) {
//       return { 
//         success: false, 
//         error: 'No fingerprint data provided. Please capture at least one finger.' 
//       };
//     }

//     console.log(`📝 Saving ${templates.length} fingerprint template(s)`);

//     // Update student record with templates
//     const student = await databases.updateDocument(
//       config.databaseId,
//       config.studentsCollectionId,
//       studentId,
...
// /**
//  * Get all students with registered fingerprints
//  * Returns formatted data ready for client-side comparison
//  * @returns {Promise<Object>} Array of templates with student info
//  */
// export const getStudentsWithFingerprints = async () => {
//   try {
//     console.log('\n📋 === FETCHING STUDENTS WITH FINGERPRINTS ===');
...
//     console.log(`Prepared ${templates.length} fingerprint templates for comparison`);
//     console.log('===============================================\n');
...
//   } catch (error) {
//     console.error('❌ Error fetching students with fingerprints:', error);
//     return {
//       success: false,
...
// /**
//  * CLIENT-SIDE ONLY: Verify fingerprint
//  * This must run in browser where DigitalPersona SDK is available
//  * @param {Object} fingerprintScannerInstance - Initialized scanner instance
//  * @returns {Promise<Object>} Match result with student data
//  */
// export const verifyStudentFingerprint = async (fingerprintScannerInstance) => {
//   try {
//     console.log('\n🔍 === FINGERPRINT VERIFICATION ===');

//     // Step 1: Fetch all registered templates
//     const templatesResult = await getStudentsWithFingerprints();

//     if (!templatesResult.success) {
//       return {
//         success: false,
//         matched: false,
//         error: 'Failed to fetch templates: ' + templatesResult.error
//       };
//     }
...
//         success: true,
//         matched: false,
//         message: 'No registered fingerprints in database'
//       };
//     }

//     console.log(`Verifying against ${templatesResult.data.length} templates`);

//     // Step 2: Capture fingerprint
//     console.log('👆 Capturing fingerprint...');
//     const captureResult = await fingerprintScannerInstance.capture('Verification');
...
//     // Step 3: Compare with all templates
//     console.log('🔄 Comparing...');
//     let bestMatch = null;
//     let highestConfidence = 0;

//     for (let i = 0; i < templatesResult.data.length; i++) {
//       const stored = templatesResult.data[i];
      
//       try {
//         const compareResult = await fingerprintScannerInstance.compareTemplates(
//           captureResult.template,
//           stored.template
//         );

//         if (compareResult.success && compareResult.matched) {
//           if (compareResult.confidence > highestConfidence) {
//             highestConfidence = compareResult.confidence;
//             bestMatch = stored;
//           }
//         }
//       } catch (err) {
//         console.warn(`Compare error for ${stored.matricNumber}:`, err.message);
//       }
//     }

//     if (bestMatch) {
//       console.log('✅ MATCH FOUND:', bestMatch.student.matricNumber);
//       console.log('=================================\n');
//       return {
//         success: true,
//         matched: true,
...
//  * CLIENT-SIDE ONLY
//  * @param {Object} fingerprintScannerInstance - Scanner instance
//  * @param {string} newTemplate - New fingerprint template to check
//  * @returns {Promise<Object>} Duplicate check result
//  */
// export const checkFingerprintDuplicate = async (fingerprintScannerInstance, newTemplate) => {
//   try {
//     console.log('\n🔍 === CHECKING FOR DUPLICATES ===');

//     if (!newTemplate || typeof newTemplate !== 'string') {
//       return { success: false, error: 'Invalid template provided' };
//     }
...
//     console.log(`Checking against ${templatesResult.data.length} existing templates...`);

//     // Compare with each existing template
//     for (const stored of templatesResult.data) {
//       try {
//         const compareResult = await fingerprintScannerInstance.compareTemplates(
//           newTemplate,
//           stored.template
...
//           console.warn(`   Registered to: ${stored.student.firstName} ${stored.student.surname}`);
//           console.warn(`   Matric: ${stored.matricNumber}`);
//           console.warn(`   Finger: ${stored.fingerName}`);
//           console.warn('================================\n');
...
//             message: `Already registered to ${stored.student.firstName} ${stored.student.surname} (${stored.matricNumber})`
//           };
//         }
//       } catch (err) {
//         console.warn('Compare error:', err.message);
//         continue;
//       }
//     }

//     console.log('✅ No duplicates - fingerprint is unique');
//     console.log('=====================================\n');
...
export const saveFingerprints = async (studentId, fingerprintData) => {
  try {
    console.log('💾 Saving fingerprint templates for student:', studentId);
    ...
    // Count captured fingerprints
    const capturedFingers = Object.values(fingerprintData).filter(
      template => template && template.trim() !== ''
    );
    
    if (capturedFingers.length === 0) {
      throw new Error('No fingerprint data provided. Please capture at least one finger.');
    }

    console.log(`📊 Saving ${capturedFingers.length} fingerprint(s)`);

    // Validate template formats (ensure they're valid Base64)
    for (const [key, template] of Object.entries(fingerprintData)) {
      if (template && template.trim() !== '') {
        // Basic Base64 validation
        if (!/^[A-Za-z0-9+/]*={0,2}$/.test(template)) {
          throw new Error(`Invalid template format for ${key}`);
        }
      }
    }

    // Update student document with fingerprint templates
    const student = await databases.updateDocument(
      config.databaseId,
      config.studentsCollectionId,
      studentId,
      {
        ...
        fingerprintsCapturedAt: new Date().toISOString(),
        fingerprintsCaptured: true,
        fingerprintCount: capturedFingers.length
      }
    );
    ...
    return { 
      success: true, 
      data: student,
      message: `${capturedFingers.length} fingerprint(s) saved successfully`
    };
...
/**
 * Get all students with captured fingerprints
...
export const getStudentsWithFingerprints = async () => {
  try {
    ...
    // Query students with fingerprints
    const studentsResponse = await databases.listDocuments(
      config.databaseId,
      config.studentsCollectionId,
      [
    ...
    // Prepare templates for client-side comparison
    const templates = [];
    ...
    console.log(`📝 Prepared ${totalTemplates} fingerprint template(s) from ${studentCount} student(s)`);
    ...
    return {
      ...
      message: `Retrieved ${totalTemplates} fingerprint templates`
    };
    ...
    return {
      success: false,
      error: error.message || 'Failed to fetch student fingerprints',
      data: [],
      total: 0
    };
  }
...
/**
 * Verify student fingerprint against database
 * Main verification function for authentication
 */
export const verifyStudentFingerprint = async (fingerprintScannerInstance) => {
  try {
    console.log('\n🔍 === FINGERPRINT VERIFICATION STARTED ===');

    // Step 1: Validate scanner instance
    if (!fingerprintScannerInstance) {
      throw new Error('Fingerprint scanner instance not provided');
    }

    // Step 2: Fetch all stored fingerprint templates
    console.log('📥 Fetching stored templates from database...');
    const templatesResult = await getStudentsWithFingerprints();

    if (!templatesResult.success) {
      throw new Error('Failed to fetch stored fingerprints: ' + templatesResult.error);
    }
    ...
    if (templatesResult.data.length === 0) {
      return {
        success: true,
        matched: false,
        message: 'No registered fingerprints found in database. Please register students first.'
      };
    }

    console.log(`📋 Loaded ${templatesResult.data.length} stored template(s) for comparison`);

    // Step 3: Capture fingerprint from scanner
    console.log('👆 Waiting for finger placement...');
    const captureResult = await fingerprintScannerInstance.capture('Verification');

    if (!captureResult.success) {
      throw new Error('Failed to capture fingerprint: ' + captureResult.error);
    }

    console.log('✅ Fingerprint captured successfully');
    console.log(`📊 Capture quality: ${captureResult.quality}%`);
    
    const capturedTemplate = captureResult.template;

    // Step 4: Compare captured fingerprint with all stored templates
    console.log('🔄 Comparing with stored templates...');
    console.log(`⏱️ Processing ${templatesResult.data.length} comparison(s)...`);
    ...
    for (let i = 0; i < templatesResult.data.length; i++) {
      ...
      try {
        ...
        if (compareResult.success && compareResult.matched) {
          console.log(`  ✅ [${i + 1}/${templatesResult.data.length}] MATCH: ${stored.matricNumber} (${stored.fingerName}) - ${compareResult.similarity?.toFixed(1)}%`);
          ...
          // Only log non-matches in verbose mode
          if (process.env.NODE_ENV === 'development') {
            console.log(`  ❌ [${i + 1}/${templatesResult.data.length}] No match: ${stored.matricNumber}`);
          }
        }
    ...
    if (bestMatch) {
      ...
      console.log(`Finger: ${bestMatch.fingerName}`);
      ...
      return {
        ...
        confidence: bestMatch.confidence?.toFixed(1) || highestSimilarity.toFixed(1),
        matchScore: lowestScore,
        fingerUsed: bestMatch.fingerName,
    ...
    console.log('\n❌ === NO MATCH FOUND ===');
    console.log('The fingerprint does not match any registered student');
    console.log('=========================\n');

    return {
      success: true,
      matched: false,
      message: 'No matching student found. Please ensure you are registered and try again.'
    };
...
/**
 * Check for duplicate fingerprints before registration
 * Prevents same finger being registered to multiple students
 */
export const checkFingerprintDuplicate = async (fingerprintScannerInstance, newTemplate) => {
  try {
    ...
    // Validate inputs
    if (!fingerprintScannerInstance || !newTemplate) {
      throw new Error('Invalid parameters for duplicate check');
    }
    ...
    if (!templatesResult.success) {
      throw new Error('Failed to fetch existing fingerprints: ' + templatesResult.error);
    }

    if (templatesResult.data.length === 0) {
      console.log('✅ No existing fingerprints - first registration');
      return {
        success: true,
        isDuplicate: false,
        message: 'No duplicates found (first registration)'
      };
    }

    console.log(`📊 Checking against ${templatesResult.data.length} existing template(s)...`);

    // Compare new template with all existing ones
    ...
    for (const stored of templatesResult.data) {
      ...
      if (compareResult.matched && compareResult.similarity >= DUPLICATE_THRESHOLD) {
        console.warn(`⚠️ DUPLICATE FOUND: ${stored.matricNumber} (${stored.fingerName})`);
        console.warn(`   Similarity: ${compareResult.similarity.toFixed(1)}%`);
        ...
        return {
          ...
          fingerName: stored.fingerName,
          similarity: compareResult.similarity,
          message: `This fingerprint is already registered to ${stored.student.firstName} ${stored.student.surname} (${stored.matricNumber}, ${stored.fingerName})`
        };
      }
    }

    console.log('✅ No duplicates found - fingerprint is unique');
    return {
      success: true,
      isDuplicate: false,
      message: 'Fingerprint is unique and can be registered'
    };
...
/**
 * Delete student fingerprints
 * Used when student needs to re-register fingerprints
 */
export const deleteFingerprints = async (studentId) => {
  try {
    console.log('🗑️ Deleting fingerprints for student:', studentId);
    ...
    return {
      success: false,
      error: error.message || 'Failed to delete fingerprints'
    };
  }
};
...
export const getFingerprintStats = async () => {
  try {
    const result = await getStudentsWithFingerprints();
    ...
    return {
      ...
      data: {
        ...
        averageTemplatesPerStudent: result.studentCount > 0 
          ? (result.total / result.studentCount).toFixed(1) 
          : 0
      }
    };
...
export const searchStudentByFace = async (capturedImageBase64) => {
  try {
    ...
    // ✅ FIXED: Use Node.js form-data package
    const FormData = require('form-data');
    const formData = new FormData();
    formData.append('api_key', FACEPP_API_KEY);
    formData.append('api_secret', FACEPP_API_SECRET);
    ...
    console.log('📤 Sending to Face++...', {
      ...
      hasSecret: !!FACEPP_API_SECRET,
      outerId: FACESET_OUTER_ID
    });
