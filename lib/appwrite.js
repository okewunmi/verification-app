...
export const createAttendanceSession = async (courseCode, courseTitle, sessionType = 'signin') => {
  try {
    ...
    const existingSession = await databases.listDocuments(
      config.databaseId,
      'attendanceSessions', // New collection for sessions
      [
        Query.equal('courseCode', courseCode),
...
export const getCoursesWithRegisteredStudents = async () => {
  try {
    ...
    registrations.documents.forEach(reg => {
      if (!coursesMap[reg.courseCode]) {
        coursesMap[reg.courseCode] = {
      ...
      coursesMap[reg.courseCode].studentCount++;
      coursesMap[reg.courseCode].students.push(reg.matricNumber);
    });
...
export const getSessionAttendanceReport = async (sessionId) => {
  try {
    ...
    const stats = {
      ...
      present: report.filter(r => r.attended).length,
      absent: report.filter(r => !r.attended).length,
      signedOut: report.filter(r => r.signedOut).length,
      attendanceRate: allStudents.length > 0 
        ? ((report.filter(r => r.attended).length / allStudents.length) * 100).toFixed(1)
        : 0
    };
