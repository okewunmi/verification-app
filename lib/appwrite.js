...
export const saveFingerprints = async (studentId, fingerprintData) => {
  try {
    ...
    console.log('📦 Received fingerprintData:', fingerprintData);
    ...
    const capturedFingers = Object.entries(fingerprintData).filter(
      ([key, template]) => {
        ...
        if (template) {
          console.warn(`⚠️ ${key} is not a string:`, typeof template);
        }
        return false;
      }
    );
    ...
    for (const finger of fingers) {
      if (finger.template && typeof finger.template === 'string' && finger.template.trim() !== '') {
        try {
          console.log(`📤 Uploading ${finger.key}...`);
          
          // ✅ SIMPLIFIED: Just use finger name and timestamp
          // The actual file ID will be generated by ID.unique()
          const fileName = `fingerprint_${finger.key}_${Date.now()}.txt`;
          ...
          // ID.unique() generates clean IDs like '692fdcea0029cfcd2df5'
          const uploadResult = await storage.createFile(
            config.bucketId,
            ID.unique(), // ✅ This is what matters - generates clean file ID
            file
          );
          
          fileIds[finger.field] = uploadResult.$id;
          console.log(`✅ ${finger.key} uploaded (File ID: ${uploadResult.$id})`);
          
        } catch (uploadError) {
          console.error(`❌ Failed to upload ${finger.key}:`, uploadError.message);
          // Don't throw - continue with other fingers
        }
      } else if (finger.template) {
        console.warn(`⚠️ Skipping ${finger.key}: template is not a string (type: ${typeof finger.template})`);
      }
    }

    // Check if at least one finger was uploaded successfully
    const uploadedCount = Object.values(fileIds).filter(id => id !== '').length;
    
    if (uploadedCount === 0) {
      throw new Error('Failed to upload any fingerprints to storage');
    }
    ...
    return { 
      ...
      data: student,
      fileIds: fileIds,
      message: `${uploadedCount} fingerprint(s) saved to storage`
    };
...
export const getFingerprintTemplate = async (fileId) => {
  try {
    ...
    // CRITICAL FIX: URL-encode the fileId to handle special characters
    ...
    console.log(`📦 Downloaded, length: ${template.length}`);
    console.log(`📦 Content (first 100): ${template.substring(0, 100)}`);
    
    // No need to extract - already stored as clean data
    console.log(`✅ Final template length: ${template.length} chars`);
    return template;
...
export const verifyStudentFingerprint = async () => {
  try {
    ...
    const captureResult = await fingerprintScanner.capture('Verification');
    ...
    // CRITICAL: Log the captured template format
    const capturedTemplate = captureResult.template;
    console.log('📦 Captured template type:', typeof capturedTemplate);
    console.log('📏 Captured template length:', capturedTemplate.length);
    
    // Check if it needs extraction
    if (capturedTemplate.startsWith('[') || capturedTemplate.startsWith('{')) {
      console.log('⚠️ Captured template appears to be in DigitalPersona JSON format');
    }
    ...
    for (let i = 0; i < templatesResult.data.length; i++) {
      ...
      console.log(`     Captured: ${capturedTemplate.length} chars, Stored: ${stored.template?.length || 0} chars`);
      ...
      try {
        ...
        if (compareResult.success) {
          ...
          if (compareResult.matched) {
            console.log(`  ✅ MATCH FOUND: ${stored.matricNumber} (${stored.fingerName})`);
    ...
    return {
      ...
      matched: false,
      confidence: 0,
      message: 'No matching student found. Highest similarity: ' + highestSimilarity.toFixed(1) + '%'
    };
...
//         const base64Data = finger.imageData;
...
//         fingerprintsCapturedAt: new Date().toISOString(),
//         fingerprintsCaptured: true,
//         fingerprintFormat: 'PNG' // Mark as PNG format
//       }
//     );
...
//       data: student,
//       fileIds: fileIds,
//       message: `${uploadedCount} fingerprint(s) saved as PNG images`
//     };
...
/**
 * Save fingerprint PNG images to Appwrite Storage
 * ROBUST VERSION with extensive debugging and error handling
 */
export const saveFingerprintsPNG = async (studentId, fingerprintData) => {
  try {
    console.log('💾 Saving PNG fingerprints for student:', studentId);
    ...
    for (const finger of fingers) {
      ...
      try {
        ...
        console.log(`🔍 ${finger.key} - Has data URL?: ${base64Data.includes('data:image')}`);
        console.log(`🔍 ${finger.key} - Has comma?: ${base64Data.includes(',')}`);
        ...
        if (base64Data.includes('data:image')) {
          ...
          if (parts.length === 2) {
            ...
            // Handle multiple commas - take everything after first comma
        ...
        // ⭐ STEP 3: Remove any newlines, carriage returns, tabs
        base64Data = base64Data.replace(/[\r\n\t]/g, '');
        
        // ⭐ STEP 4: Trim any remaining whitespace
        base64Data = base64Data.trim();
        
        console.log(`✅ ${finger.key} - Cleaned length: ${base64Data.length}`);
        console.log(`✅ ${finger.key} - First 50 cleaned: "${base64Data.substring(0, 50)}"`);
        console.log(`✅ ${finger.key} - Last 50 cleaned: "${base64Data.substring(base64Data.length - 50)}"`);
        
        // ⭐ STEP 5: Validate base64 format
        const base64Regex = /^[A-Za-z0-9+/]*={0,2}$/;
        if (!base64Regex.test(base64Data)) {
          console.error(`❌ ${finger.key} - Invalid base64 format detected!`);
        ...
        console.log(`✅ ${finger.key} - Base64 format validated`);
        
        // ⭐ STEP 6: Try to decode base64 to verify it's valid
        let byteString;
        try {
          ...
          console.error(`   Base64 sample (first 200 chars): "${base64Data.substring(0, 200)}"`);
          throw new Error(`Failed to decode base64 for ${finger.key}: ${decodeError.message}`);
        }
        
        // ⭐ STEP 7: Convert to ArrayBuffer
        const ab = new ArrayBuffer(byteString.length);
        const ia = new Uint8Array(ab);
    ...
    console.log(`✅ ${uploadedCount} fingerprint(s) uploaded successfully`);
