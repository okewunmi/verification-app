...
let _fingerprintCache = null;          // Cached result of getStudentsWithFingerprintsPNG
let _fingerprintCacheTime = 0;         // When it was last populated
const FINGERPRINT_CACHE_TTL = 10 * 60 * 1000; // 10 minutes

/**
 * Invalidate the module-level fingerprint cache.
 * Call this after saveFingerprintsPNG() so the next enrollment
 * session picks up the newly saved fingerprints.
 */
export const invalidateFingerprintCache = () => {
  _fingerprintCache = null;
  _fingerprintCacheTime = 0;
  console.log('🗑️ Fingerprint cache invalidated');
};
...
export const permanentlyDeleteStudent = async (documentId) => {
  try {
    ...
    // Delete profile picture from storage
    // Delete old picture if exists
if (student.success && student.data.profilePictureUrl) {
  // Extract file ID from URL if needed, or skip deletion
  // Since we don't store fileId, we can't delete old files automatically
  console.warn('⚠️ Old profile picture not deleted (no fileId stored)');
}
...
// ============================================
// OPTIMIZED APPWRITE FUNCTIONS - PNG FINGERPRINT STORAGE
// ⚡ Features: Parallel fetching, better error handling, performance metrics
// ============================================

/**
 * Get PNG fingerprint image from storage
 * (Keep this function as-is - it's already optimized)
 */
...
//     // Fetch all students with fingerprints
//     const studentsResponse = await databases.listDocuments(
//       config.databaseId,
//       config.studentsCollectionId,
//       [
...
//     // ⭐ PARALLEL FETCHING - Create all promises first
//     const fetchPromises = [];
//     let expectedFingerprints = 0;
...
//         // Create promise for parallel fetch
//         // Each promise fetches the image and returns complete fingerprint object
//         const fetchPromise = getFingerprintImagePNG(finger.fileId)
//           .then(imageData => {
//             // Only return object if fetch succeeded
//             if (imageData) {
//               return {
//                 id: student.$id,
...
//             console.warn(`⚠️ Failed to fetch ${finger.name} for ${student.matricNumber}:`, error.message);
...
//     console.log(`🔄 Fetching ${expectedFingerprints} fingerprint image(s) in parallel...`);
    
//     // ⭐ Wait for ALL fetches to complete in parallel
//     const results = await Promise.all(fetchPromises);
    
//     // Filter out failed fetches (null values)
//     const validFingerprints = results.filter(r => r !== null);
    
//     const elapsedTime = ((Date.now() - startTime) / 1000).toFixed(2);
//     const successRate = ((validFingerprints.length / expectedFingerprints) * 100).toFixed(1);
    
//     console.log(`📝 Loaded ${validFingerprints.length}/${expectedFingerprints} PNG image(s) (${successRate}% success)`);
//     console.log(`⚡ Total time: ${elapsedTime}s`);

//     return {
//       success: true,
//       data: validFingerprints,
//       total: validFingerprints.length,
//       studentCount: studentCount,
//       expectedCount: expectedFingerprints,
//       successRate: parseFloat(successRate),
//       loadTime: parseFloat(elapsedTime),
//       message: `Retrieved ${validFingerprints.length} fingerprint images in ${elapsedTime}s`
//     };
...
// };
// ============================================================
// REPLACE the existing getStudentsWithFingerprintsPNG function
// in lib/appwrite.js with this version.
//
// Key changes:
//  1. Uses module-level cache (_fingerprintCache) — fetches once per session
//  2. Returns cached result instantly on fingers 2-5 of same enrollment
//  3. saveFingerprintsPNG() now calls invalidateFingerprintCache() so the
//     next enrollment session always sees fresh data
// ============================================================

export const getStudentsWithFingerprintsPNG = async ({ forceRefresh = false } = {}) => {
  const now = Date.now();
  ...
  if (
    !forceRefresh &&
    _fingerprintCache !== null &&
    (now - _fingerprintCacheTime) < FINGERPRINT_CACHE_TTL
  ) {
    console.log(`💾 Fingerprint cache HIT (${_fingerprintCache.data.length} fingerprints, age ${Math.round((now - _fingerprintCacheTime) / 1000)}s)`);
    return _fingerprintCache;
  }

  // ── Cache miss — fetch from Appwrite ─────────────────────────────────────
  try {
    console.log('📋 Fetching students with PNG fingerprints (cache miss)...');
    const startTime = Date.now();
    ...
    if (studentCount === 0) {
      ...
      _fingerprintCache = emptyResult;
      _fingerprintCacheTime = now;
      return emptyResult;
    }

    // Build all fetch promises in one pass
    const fetchPromises = [];
    let expectedFingerprints = 0;
    ...
    for (const student of studentsResponse.documents) {
      ...
      for (const finger of fingers) {
        if (!finger.fileId?.trim()) continue;
        expectedFingerprints++;

        const promise = getFingerprintImagePNG(finger.fileId)
          .then(imageData => {
          ...
          .catch(err => {
            console.warn(`⚠️ Failed to fetch ${finger.name} for ${student.matricNumber}:`, err.message);
            return null;
          });
    ...
    console.log(`🔄 Fetching ${expectedFingerprints} fingerprint image(s) in parallel...`);
    const results = await Promise.all(fetchPromises);
    const validFingerprints = results.filter(Boolean);

    const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);
    const successRate = expectedFingerprints > 0
      ? ((validFingerprints.length / expectedFingerprints) * 100).toFixed(1)
      : '100';

    console.log(`📝 Loaded ${validFingerprints.length}/${expectedFingerprints} PNG images (${successRate}%) in ${elapsed}s`);
    ...
    const result = {
      ...
      message: `Retrieved ${validFingerprints.length} fingerprint images in ${elapsed}s`,
    };

    // ── Populate cache ──────────────────────────────────────────────────────
    _fingerprintCache = result;
    _fingerprintCacheTime = Date.now();
    console.log('💾 Fingerprint cache populated');
 ...
 * (Keep existing implementation - it's already good)
 */
...
// ============================================
// OPTIMIZED APPWRITE FACE RECOGNITION FUNCTIONS
// ============================================

/**
 * ⭐ OPTIMIZED: Get all students who have face descriptors stored
 * Used during verification to compare against database
 * 
 * Improvements:
 * - Better error handling and validation
 * - Filters invalid descriptors
...
export const getStudentsWithFaceDescriptors = async () => {
  try {
    ...
    console.log(`✅ Found ${response.documents.length} student(s) with face data`);

    // Filter out invalid descriptors with detailed logging
    const validStudents = [];
    const invalidStudents = [];

    for (const student of response.documents) {
      // Check if faceDescriptor exists and is not empty
      ...
      try {
        ...
        if (descriptor.length !== 128) {
          invalidStudents.push({
            matricNumber: student.matricNumber,
            reason: `Invalid length: ${descriptor.length} (expected 128)`
          });
          continue;
        }
    ...
    // Log invalid descriptors if any
    if (invalidStudents.length > 0) {
      console.warn(`⚠️ Found ${invalidStudents.length} invalid descriptor(s):`);
      invalidStudents.forEach(inv => {
        console.warn(`   - ${inv.matricNumber}: ${inv.reason}`);
      });
    }

    console.log(`📊 Valid descriptors: ${validStudents.length}/${response.documents.length}`);
    console.log(`⚡ Load time: ${elapsedTime}s`);
 ...
 * 
 * Improvements:
 * - Validates descriptor before saving
...
export const updateStudentFaceDescriptor = async (studentId, descriptor, confidence) => {
  try {
    ...
    if (!Array.isArray(descriptor) || descriptor.length !== 128) {
      throw new Error(`Invalid descriptor: expected 128-element array, got ${descriptor?.length || 'null'}`);
    }
    ...
    console.log(`💾 Saving face descriptor for student ${studentId}...`);
    ...
    await databases.updateDocument(
      ...
      studentId,
      {
        ...
        faceConfidence: String(confidence || 0)  // ⭐ CONVERT TO STRING
      }
    );

    console.log(`✅ Face descriptor saved for student ${studentId} (confidence: ${confidence}%)`);
...
export const removeStudentFaceDescriptor = async (studentId) => {
  try {
    ...
    console.log(`🗑️ Removing face descriptor for student ${studentId}...`);
...
export const getFaceEnrollmentStats = async () => {
  try {
    console.log('📊 Fetching face enrollment statistics...');
    ...
    console.log(`📈 Enrollment stats: ${enrolled}/${total} (${percentage}%)`);
...
/**
 * ⭐ NEW: Batch validate face descriptors
 * Useful for checking data integrity
 */
export const validateAllFaceDescriptors = async () => {
  try {
    console.log('🔍 Validating all face descriptors...');
      ...
      try {
        ...
        if (!Array.isArray(descriptor) || descriptor.length !== 128) {
          ...
          results.issues.push({
            matricNumber: student.matricNumber,
            studentId: student.$id,
            issue: `Invalid length: ${descriptor?.length || 'N/A'}`
          });
          continue;
        }
    ...
    console.error('❌ Error validating descriptors:', error);
    return {
      success: false,
      error: error.message
    };
