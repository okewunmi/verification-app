...
/**
 * Save fingerprints to storage (uses same bucket as profile pictures)
 * Files are organized by naming: {studentId}_fingerprint_{finger}.txt
 */
...
//       ([key, template]) => template && template.trim() !== ''
//     );
...
//     console.log(`📊 Saving ${capturedFingers.length} fingerprint(s) to storage...`);
...
//     const fingers = [
//       { key: 'thumb', field: 'thumbFileId', template: fingerprintData.thumb },
//       { key: 'index', field: 'indexFileId', template: fingerprintData.index },
//       { key: 'middle', field: 'middleFileId', template: fingerprintData.middle },
//       { key: 'ring', field: 'ringFileId', template: fingerprintData.ring },
//       { key: 'pinky', field: 'pinkyFileId', template: fingerprintData.pinky }
//     ];

//     for (const finger of fingers) {
//       if (finger.template && finger.template.trim() !== '') {
//         try {
//           console.log(`📤 Uploading ${finger.key}...`);
          
//           // CRITICAL: Use naming convention to organize files in shared bucket
//           // Format: {studentId}_fingerprint_{finger}.txt
//           const fileName = `${studentId}_fingerprint_${finger.key}.txt`;
          
//           const blob = new Blob([finger.template], { type: 'text/plain' });
//           const file = new File([blob], fileName, { type: 'text/plain' });
          
//           // Upload to SHARED bucket (same as profile pictures)
//           const uploadResult = await storage.createFile(
//             config.storageBucketId, // Same bucket ID
//             ID.unique(),
//             file
//           );
          
//           fileIds[finger.field] = uploadResult.$id;
//           console.log(`✅ ${finger.key} uploaded (ID: ${uploadResult.$id})`);
...
//     // Update student document with file IDs
//     const student = await databases.updateDocument(
//       config.databaseId,
//       config.studentsCollectionId,
//       studentId,
...
//     console.log('✅ All fingerprints saved to storage successfully');
    
//     return { 
//       success: true, 
//       data: student,
//       fileIds: fileIds,
//       message: `${capturedFingers.length} fingerprint(s) saved to storage`
//     };
...
export const saveFingerprints = async (studentId, fingerprintData) => {
  try {
    ...
    // Log all template values to debug
    console.log('🔍 Inspecting template data:');
    Object.entries(fingerprintData).forEach(([key, value]) => {
      console.log(`  ${key}:`, {
        type: typeof value,
        isString: typeof value === 'string',
        value: value ? value.substring(0, 50) + '...' : 'null/empty',
        length: value ? value.length : 0
      });
    });

    // Fix: Use proper string checking
    const capturedFingers = Object.entries(fingerprintData)
      .filter(([key, template]) => {
        // Check if template is truthy and is a string
        ...
        return templateStr.trim() !== '';
      });
    
    console.log(`📊 Found ${capturedFingers.length} valid finger(s) to save`);
    
    if (capturedFingers.length === 0) {
      throw new Error('No valid fingerprint data provided. Please capture at least one finger.');
    }
    ...
    for (const finger of fingers) {
      try {
        // Fix: Convert to string and check if valid
        let templateStr = '';
        
        if (finger.template) {
          templateStr = typeof finger.template === 'string' 
            ? finger.template 
            : String(finger.template);
        }
        
        if (templateStr && templateStr.trim() !== '') {
          console.log(`📤 Uploading ${finger.key}...`);
          console.log(`📄 Template type: ${typeof finger.template}`);
          console.log(`📄 Template length: ${templateStr.length} chars`);
          console.log(`📄 First 100 chars: ${templateStr.substring(0, 100)}...`);
          
          // CRITICAL: Use naming convention to organize files in shared bucket
          const fileName = `${studentId}_fingerprint_${finger.key}.txt`;
          
          const blob = new Blob([templateStr], { type: 'text/plain' });
          const file = new File([blob], fileName, { type: 'text/plain' });
          ...
          console.log(`⏭️ Skipping ${finger.key} - no valid template data`);
        }
      } catch (uploadError) {
        console.error(`❌ Failed to upload ${finger.key}:`, uploadError.message);
    ...
    const uploadedFiles = Object.values(fileIds).filter(id => id && id.trim() !== '');
    
    if (uploadedFiles.length === 0) {
      throw new Error('No fingerprint files were uploaded successfully');
    }

    // Update student document with file IDs
    console.log('📝 Updating student document with file IDs:', fileIds);
    const student = await databases.updateDocument(
      config.databaseId,
      config.studentsCollectionId,
      studentId,
    ...
    console.log('✅ All fingerprints saved to storage successfully');
    console.log(`📊 Total uploaded: ${uploadedFiles.length} file(s)`);
    
    return { 
      success: true, 
      data: student,
      fileIds: fileIds,
      message: `${uploadedFiles.length} fingerprint(s) saved to storage`
    };
...
// Utility function to log and validate fingerprint data
export const debugFingerprintData = (fingerprintData, label = 'Fingerprint Data') => {
  console.log(`\n🔍 === ${label} DEBUG ===`);
  ...
  if (typeof fingerprintData !== 'object') {
    console.log(`❌ Data is not an object: ${typeof fingerprintData}`);
    return;
  }
...
// Then in your page.js, before calling saveFingerprints:
debugFingerprintData(fingerprintData, 'Before Saving');
/**
 * Get fingerprint template from storage
 * FIXED: Proper Blob handling with fallback
 */
// export const getFingerprintTemplate = async (fileId) => {
//   try {
//     if (!fileId || fileId.trim() === '') {
//       return null;
//     }

//     console.log(`📥 Fetching template from storage (${fileId})...`);
    
//     try {
//       // Try getFileDownload first
//       const result = await storage.getFileDownload(config.bucketId, fileId);
      
//       // Handle different return types
//       if (result instanceof Blob) {
//         const template = await result.text();
//         console.log(`✅ Template loaded from Blob (${template.length} chars)`);
//         return template;
//       } else if (typeof result === 'string') {
//         console.log(`✅ Template loaded as string (${result.length} chars)`);
//         return result;
//       } else {
//         throw new Error('Unexpected result type from getFileDownload');
//       }
//     } catch (downloadError) {
//       // Fallback: Try getFileView and fetch
//       console.log('⚠️ getFileDownload failed, trying getFileView...');
//       const fileUrl = storage.getFileView(config.bucketId, fileId);
//       const response = await fetch(fileUrl.href || fileUrl);
//       const template = await response.text();
//       console.log(`✅ Template loaded via fetch (${template.length} chars)`);
//       return template;
//     }
...
export const getFingerprintTemplate = async (fileId) => {
  try {
    if (!fileId || fileId.trim() === '') {
      console.log('⚠️ No file ID provided');
      return null;
    }
    ...
    try {
      // Method 1: Try getFileDownload (returns a Blob in newer SDK)
      const result = await storage.getFileDownload(config.bucketId, fileId);
      
      console.log('🔍 getFileDownload result:', {
        type: typeof result,
      ...
      if (result instanceof Blob) {
        ...
        console.log('⚠️ Unexpected result type, trying to stringify...');
        return String(result);
      }
    } catch (downloadError) {
      console.log('⚠️ getFileDownload failed, trying getFileView...');
      
      // Method 2: Fallback to getFileView
      const fileUrl = storage.getFileView(config.bucketId, fileId);
      
      console.log('🔗 File URL:', fileUrl);
      
      // Handle if fileUrl is an object with href property
      const url = fileUrl.href || fileUrl;
    ...
    console.error('❌ Error fetching template:', error.message);
    console.error('File ID:', fileId);
    console.error('Bucket ID:', config.bucketId);
...
export const getStudentsWithFingerprints = async () => {
  try {
    ...
    for (const student of studentsResponse.documents) {
      ...
      for (const finger of fingers) {
        if (finger.fileId && finger.fileId.trim() !== '') {
          const template = await getFingerprintTemplate(finger.fileId);
    ...
    console.log(`📝 Loaded ${totalTemplates} template(s) from storage`);
    ...
    return {
      ...
      message: `Retrieved ${totalTemplates} fingerprint templates from storage`
    };
...
/**
 * Delete fingerprints from storage
 */
export const deleteFingerprints = async (studentId) => {
  try {
    ...
    const fileIds = [
      currentStudent.thumbTemplate,
      currentStudent.indexTemplate,
      currentStudent.middleTemplate,
      currentStudent.ringTemplate,
      currentStudent.pinkyTemplate
    ];
    ...
    for (const fileId of fileIds) {
      if (fileId && fileId.trim() !== '') {
        try {
          ...
          console.warn(`⚠️ Could not delete file ${fileId}:`, deleteError.message);
        }
      }
    }
    ...
    console.log('✅ Fingerprints deleted from storage successfully');
...
export const verifyStudentFingerprint = async (fingerprintScannerInstance) => {
  try {
    ...
    console.log('📥 Fetching stored templates from storage...');
    ...
    if (templatesResult.data.length === 0) {
      return {
        success: true,
        matched: false,
        message: 'No registered fingerprints found. Please register students first.'
      };
    }

    console.log(`📋 Loaded ${templatesResult.data.length} template(s)`);
    ...
    console.log('✅ Captured successfully (Quality: ' + captureResult.quality + '%)');
    ...
    for (let i = 0; i < templatesResult.data.length; i++) {
      ...
      try {
        ...
        if (compareResult.success && compareResult.matched) {
          console.log(`  ✅ MATCH: ${stored.matricNumber} (${stored.fingerName})`);
        ...
        console.warn(`  ⚠️ Error comparing with ${stored.matricNumber}`);
        continue;
      }
    }
...
export const checkFingerprintDuplicate = async (fingerprintScannerInstance, newTemplate) => {
  try {
    ...
    console.log(`📊 Checking against ${templatesResult.data.length} template(s)...`);
    ...
    for (const stored of templatesResult.data) {
      ...
      if (compareResult.matched && compareResult.similarity >= DUPLICATE_THRESHOLD) {
        console.warn(`⚠️ EXACT DUPLICATE: ${stored.matricNumber} (${stored.fingerName})`);
        ...
        return {
          ...
          message: `This exact image is already registered to ${stored.student.firstName} ${stored.student.surname}`
        };
      }
    }
